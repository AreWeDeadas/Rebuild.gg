...
--
-- custom hud system start
do
    -- управление оригинальным HUD (Bar в MainScreenGui)
    library.original_hud = library.original_hud or {
        Storage = nil,
        Connection = nil,
    }

    function library.original_hud:get_bar()
    ...
    function library.original_hud:get_storage()
        local pg = lp and lp:FindFirstChild("PlayerGui")
        if not pg then return nil end
        local folder = pg:FindFirstChild("_LibraryHUD_Storage")
        if not folder then
            folder = Instance.new("Folder")
            folder.Name = "_LibraryHUD_Storage"
            folder.Parent = pg
        end
        return folder
    end

    function library.original_hud:enable()
        ...
        if pg and storage then
            local msg = pg:FindFirstChild("MainScreenGui")
            if msg then
                for _, child in ipairs(msg:GetChildren()) do
            ...
            self.Connection = pg.DescendantAdded:Connect(function(obj)
                if not (obj and obj.Name == "Bar") then return end
                local anc = obj:FindFirstAncestor("MainScreenGui")
                if not anc then return end
                pcall(function()
                    obj.Parent = self:get_storage()
                end)
            end)
        end
    end

    function library.original_hud:disable()
        if self.Connection then
            self.Connection:Disconnect()
            self.Connection = nil
        end
        ...
        if pg and storage then
            ...
            if msg then
                for _, child in ipairs(storage:GetChildren()) do
                    if child and child:IsA("GuiObject") and child.Name == "Bar" then
                        pcall(function()
                            child.Parent = msg
                        end)
                    end
    ...
    function library.original_hud:toggle(state)
        if state then
            self:enable()
        else
            self:disable()
    ...
    -- публичный API: кастомный HUD
    function library:custom_hud(options)
        local cfg = {
            enabled = false,
            selected = (options and options.type) or "Rust",
            sg = nil,
            holder = nil,
            connections = {},
        }

        -- утилита: очистка соединений
        function cfg:_disconnect()
        ...
        -- утилита: создать бар
        function cfg:_create_bar(parent, label_text, _icon_id, color3)
            ...
            -- минимальная полоса без иконок и шапки
            local line = library:create("Frame", {
                Parent = bar,
                BackgroundColor3 = themes.preset.inline,
                BorderSizePixel = 0,
            ...
            return {root = bar, line = line, fill = fill, text = text}
        end

        -- утилита: обновить бар
        function cfg:_set_bar(widget, val, max)
            val = math.clamp(tonumber(val) or 0, 0, tonumber(max) or 0)
            max = math.max(tonumber(max) or 0, 0)
            local pct = (max > 0) and (val / max) or 0
            widget.fill.Size = dim2(pct, 0, 1, 0)
            widget.text.Text = string.format("%s: %d/%d", widget.text.Text:match("^(.-):") or "Stat", val, max)
        end

        -- сборка Rust HUD
        function cfg:_create_rust()
            if self.sg then self.sg:Destroy() self.sg = nil end

            local sg = library:create("ScreenGui", {
                Parent = lp:WaitForChild("PlayerGui"),
                Name = "RustHealthArmorBars",
            ...
            layout = layout -- suppress unused

            local health = self:_create_bar(holder, "Health", 10723406795, Color3.fromRGB(185, 91, 81))
            local armor  = self:_create_bar(holder, "Armor", 10734951684, Color3.fromRGB(79, 140, 178))

            -- drag
            local dragging, drag_start, start_pos
            local uis = game:GetService("UserInputService")
            table.insert(self.connections, holder.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    dragging = true
            ...
            table.insert(self.connections, holder.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    dragging = false
                end
            end))
            ...
            table.insert(self.connections, uis.InputChanged:Connect(function(input)
                if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
                    ...
                    holder.Position = dim2(start_pos.X.Scale, start_pos.X.Offset + delta.X, start_pos.Y.Scale, start_pos.Y.Offset + delta.Y)
                end
            end))
            ...
            -- начальная установка значений (если есть Humanoid)
            local char = lp.Character or lp.CharacterAdded:Wait()
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            if hum then
                self:_set_bar(health, hum.Health, hum.MaxHealth)
            end
            -- Placeholder для брони (если у игры есть значение брони, пользователь обновит колбэком)
            self:_set_bar(armor, 0, 100)
        end

        function cfg:enable()
            if self.enabled then return end
            library.original_hud:enable()
            self:_disconnect()
            self:_create_rust()

            -- хуки для обновления здоровья
            local function bind_humanoid(hum)
                if not hum then return end
                table.insert(self.connections, hum.HealthChanged:Connect(function()
                    self:_set_bar(self.health, hum.Health, hum.MaxHealth)
                end))
                table.insert(self.connections, hum:GetPropertyChangedSignal("MaxHealth"):Connect(function()
                    self:_set_bar(self.health, hum.Health, hum.MaxHealth)
                end))
            end

            local char = lp.Character or lp.CharacterAdded:Wait()
            bind_humanoid(char and char:FindFirstChildOfClass("Humanoid"))
            table.insert(self.connections, lp.CharacterAdded:Connect(function(nc)
        ...
        -- внешнее обновление баров
        function cfg:update_health(value, max_value)
            if self.health then self:_set_bar(self.health, value, max_value) end
        end

        function cfg:update_armor(value, max_value)
            if self.armor then self:_set_bar(self.armor, value, max_value) end
        end
